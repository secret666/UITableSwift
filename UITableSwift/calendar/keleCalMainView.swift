...
class KeleCalMainView: UIView, UIScrollViewDelegate {
    ...
    override init(frame:CGRect) {
        ...
        let frame = CGRectMake(0, 80, width, itemHeight*6)
...
//        _scrollView.backgroundColor = UIColor.redColor();
    ...
    func rightBtnClick(sender:UIButton) {
        ...
        if _lastContentOffset > _scrollView.contentOffset.x {
            _direction = .Right
...
//            print("tag:\(_viewCache[i]?.tag)=\(_frame.width * CGFloat(i))--\(i)")
        }
